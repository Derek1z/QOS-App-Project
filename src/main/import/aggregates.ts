...
async function recomputeCellWeekly(conn: DuckDBConnection, idList: string): Promise<void> {
  ...
  await conn.run(`
    ...
    SELECT
      ...
      count(*) FILTER (WHERE f.prb_utilization >= r.prb_threshold_pct) >= max(r.weekly_breach_days) AS is_nc
...
async function recomputeEntityDaily(conn: DuckDBConnection, entity: string, idList: string): Promise<void> {
  ...
  await conn.run(`
    ...
    SELECT
      ...
      count(DISTINCT f.cell_id) FILTER (WHERE f.prb_utilization >= r.prb_threshold_pct) AS nc_cells,
      ROUND(100.0 * count(DISTINCT f.cell_id) FILTER (WHERE f.prb_utilization >= r.prb_threshold_pct)
