  ...
  await conn.run(`
    ...
    FROM (
      SELECT *, coalesce(
        ...
        try_strptime(date_raw, '%d/%m/%Y'),
        try_strptime(date_raw, '%d-%b-%Y'),
        try_strptime(date_raw, '%Y/%m/%d'),
        try_strptime(date_raw, '%Y%m%d')
      ) AS parsed_date
      FROM stg_import
    ) p
    LEFT JOIN dim_date d ON d.date = p.parsed_date
  `)
}
...
async function upsertDistricts(conn: DuckDBConnection): Promise<void> {
  await conn.run(`
    ...
    SELECT
      ...
      d.district_raw,
      (SELECT r.region_id FROM dim_region r WHERE r.name = d.region_raw)
    FROM (SELECT DISTINCT district_raw, region_raw FROM stg_clean
          WHERE district_raw IS NOT NULL AND district_raw <> '') d
...
async function upsertSites(conn: DuckDBConnection): Promise<void> {
  await conn.run(`
    ...
    SELECT
      ...
      s.site_raw,
      (SELECT d.district_id FROM dim_district d WHERE d.name = s.district_raw)
    FROM (SELECT DISTINCT site_raw, district_raw FROM stg_clean
          WHERE site_raw IS NOT NULL AND site_raw <> '') s
...
async function upsertCells(conn: DuckDBConnection): Promise<void> {
  await conn.run(`
    ...
    SELECT
      ...
      c.cell_name,
      (SELECT s.site_id FROM dim_site s WHERE s.name = c.site_raw),
      (SELECT d.district_id FROM dim_district d WHERE d.name = c.district_raw),
      (SELECT r.region_id FROM dim_region r WHERE r.name = c.region_raw)
    FROM (SELECT DISTINCT cell_name, site_raw, district_raw, region_raw FROM stg_clean
          WHERE cell_name IS NOT NULL AND cell_name <> '') c
...
export async function runImportCore(
  ...
  await stageCsv(conn, path, header, mapping)
